...
The first inner five-segment theorem of Tarski's geometry was first automatically proved by 
E~\cite{Sch13-LPAR} in 2019, after first being posed by Quaife in 1989~\cite{Qua89}.
Problems from the book are in the set theory ({\tt SET}), general algebra ({\tt ALG}), number 
...
John Slaney used model finding ATP systems to investigate the existence of quasigroups that
satisfy certain additional conditions~\cite{SFS95}
...
justifications, which have been checked by the Mizar proof checker.
In order to cross-verify proofs in the MML, a selection of problems and proofs was exported to
the TPTP format~\cite{US09}.
The proofs were verified using the GDV proof verifier~\ref{Sut06}.
